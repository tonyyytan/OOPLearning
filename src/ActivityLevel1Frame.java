 ...
 * Title of Class: Activity Level 1 Frame
 * 
 * Description:
...
public class ActivityLevel1Frame extends JFrame implements ActionListener {
	...
	String[] words = { "Object", "Template Class", "Constructor", "Method", "State", "Behavior" };

	// Array holding the corresponding definitions for each word
	String[] definitions = { "Attributes of object", "Initializes new objects", "Actions of object",
			"Function in a class", "Blueprint for objects", "Instance of a class" };
	...
	private void generateDefinitionButtons() {
		...
		for (int row = 0; row < definitionButtonArray.length; row++) {
			for (int col = 0; col < definitionButtonArray[row].length; col++) {
				...
				definitionLabels[row][col] = new JLabel(definitions[count]); // Correct index
	...
	private void addActionListener(ActivityLevel1Frame activityLevel1Frame) {
	...
	public void actionPerformed(ActionEvent event) {
		...
		//If the finish frame button is clicked
			...
			if (numberCorrect == 6) {
				...
				//If the user is below activity level 1
				if (activityLevel < 1) {
					
					//User gets an increase in level to level 1
					FileInput.userDataArray.get(SignUpFrame.userIdentity).setActivityLevel(1);

					//Displays a message saying completion
					JOptionPane.showMessageDialog(this, "Nice, you completed the level!", "Exiting...",
							JOptionPane.INFORMATION_MESSAGE);
					
					//Redirects page to Activity frame
					dispose();
					ActivityFrame activityFrame = new ActivityFrame();

				}

				//If user has already completed this level before
				else {
			...
			//If the user has not matched all the definitions yet
			else {
	...
	private void checkMatch() {
		...
		if (selectedDefinition != null && selectedWord != null) {
			...
			if (matches(selectedDefinition, selectedWord)) {
				...
				//Plays the sound for correct
				playSound("correctSound.wav");
				
				//Makes the definition button and word button disappear
	...
	//Method to check if the answers match up
	private boolean matches(String selectedDefinition, String selectedWord) {
		...
		if ((selectedDefinition.equals("Blueprint for objects") && selectedWord.equals("Template Class"))
			...
			//If they do match up, return true
			return true;
		}
		
		//if they don't (incorrect answer), return false
		return false;
	}
}
